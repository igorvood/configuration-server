...
--   p_key_values - значение полей, входящих в состав уникального ключа (следуют через разделитель - ',')
    function get_clob_hash(p_table_name in varchar, p_clob_name in varchar, p_unique_key in varchar,
                           p_key_values in varchar)
        return varchar;

    function get_view_src_hash(in_view_owner varchar, in_view_name varchar) return varchar;
    ...
    procedure apply_rename_command(in_object_type varchar, in_current_name varchar, in_new_name varchar);
...
create or replace package body test_util is
    ...
    procedure modify_constraints(in_cnstr_list cmn_str_tab, in_option varchar)
    is
    begin
